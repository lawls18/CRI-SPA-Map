...
strainInfo_inFile <- 'data/SupTable2_CRI_SPA_Map_Isolates.xlsx' ## table with details on each isolate
...
strainInfo <- strainInfo %>%
   ...
   mutate(aux_removed = (orf == 'YNL268W' | (lost_in_streaking == 'yes' & !(is.na(lost_in_streaking))) | !(unexpected_auxotrophy == 'no' | is.na(unexpected_auxotrophy) | onlyLeu == TRUE)), ## column marking isolates removed due to unexpected auxotrophies
          ...
          local_orf_mapping_removed = (repair_type != 'S' | phenotyped_liquid == 'no' | aux_removed | preSeq_removed | YKO_removed), ## column marking isolates to be removed from Fig 3
          distal_orf_mapping_removed = (repair_type != 'R' | phenotyped_liquid == 'no' | aux_removed | preSeq_removed | YKO_removed), ## column marking isolates to be removed from Fig 3
          local_snp_removed = (repair_type != 'S' | phenotyped_liquid == 'no' | sequenced_illumina == 'no' | aux_removed | 'yes' %in% c(het_reads, aneuploidy, recombination_elsewhere) | orf == 'YNL079C'), ## column marking isolates to be removed from Fig 4 and SFig 8
          distal_snp_removed = (repair_type != 'R' | phenotyped_liquid == 'no' | sequenced_illumina == 'no' | aux_removed | 'yes' %in% c(het_reads, aneuploidy, recombination_elsewhere) | orf == 'YNL079C')) ## column marking isolates to be removed from Fig 4 and SFig 8
